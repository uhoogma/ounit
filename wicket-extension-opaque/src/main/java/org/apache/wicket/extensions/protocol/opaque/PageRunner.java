        ...
        try {
            ...
            if (session.isClosed() && rv instanceof ProcessReturn) {
                ...
                if (session.getSessionId() != null) {
                    ((ProcessReturn) rv).setProgressInfo("You have 0 attempts left");
                    ((ProcessReturn) rv).setXHTML("Your answer was submitted too late");
                    WicketOpaqueService.staleSessions.remove(session.getSessionId());
