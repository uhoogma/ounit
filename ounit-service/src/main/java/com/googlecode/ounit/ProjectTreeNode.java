...
/**
 * A node of a project view tree. Also acts as a model to load and save file
 * data.
 *
 * @author anttix
 *
...
public class ProjectTreeNode implements Serializable, Comparable<ProjectTreeNode> {
    ...
    ProjectTreeNode(String name, File pathname) {
        ...
        // initialize static HashMap entry
        String[] split = pathname.toString().split(File.separator);
        String sessionID = "";
        for (int i = 0; i < split.length - 2; i++) {
            if (split[i].equals("ounit-work") && split[i + 1].equals("sessions")) {
                sessionID = split[i + 2];
            }
        }
        WicketOpaqueService.staleSessions.put(sessionID, false);
    ...
    /**
     * Sort directories first, if both are dirs or files, sort lexiographically
     * by name
     */
    ...
    public String getFileContents() throws IOException {
        ...
        if (WicketOpaqueService.staleSessions.get(getSessionId()) == true) {
            return "";
        } else {
