...
/**
 * Load surefire reporting plugin via reflection API to use it for parsing test
 * reports generated by surefire and failsafe plugins. This requires an active
 * Maven session and an instance of Plugin Manager to work.
 *
 * @author anttix
 *
 */
public class ReflectiveSurefireReportParser implements ReportParser {
    ...
    public TestResults parseReportFiles(List<File> dirs) throws Exception {
        ...
        for (Object f : failures) {
            ...
            String type = (String) getTestCaseProperty("FailureType", f);
            String message = (String) getTestCaseProperty("FailureMessage", f);

            results.addFailure(new FailureDetail(name, fullClassName, type, message));
        }
